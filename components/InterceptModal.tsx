...
import {
  ShieldAlert, AlertTriangle, X, Brain, Zap,
  ChevronRight, Loader2, CheckCircle2, TrendingDown,
  Briefcase, Clock, DollarSign, BookOpen, ShoppingBag,
} from "lucide-react";
...
function daysAfterSpend(freeCash: number, amount: number, weeklyBurn = 7000) {
...
function severityConfig(agentMode?: string, stressScore = 0) {
  if (agentMode === "EMERGENCY") return {
    bg: "bg-red-50", border: "border-red-300",
    headerBg: "bg-red-600", badge: "bg-red-100 text-red-700",
    icon: <ShieldAlert className="h-5 w-5 text-white" />,
    label: "EMERGENCY", labelColor: "text-red-600",
    btnPrimary: "bg-emerald-500 hover:bg-emerald-600",
    btnOverride: "border-red-200 text-red-500 hover:bg-red-50",
    barColor: "bg-red-500",
  };
  if (agentMode === "SURVIVAL") return {
    bg: "bg-orange-50", border: "border-orange-200",
    headerBg: "bg-orange-500", badge: "bg-orange-100 text-orange-700",
    icon: <AlertTriangle className="h-5 w-5 text-white" />,
    label: "SURVIVAL MODE", labelColor: "text-orange-600",
    btnPrimary: "bg-emerald-500 hover:bg-emerald-600",
  ...
  if (stressScore >= 60) return {
    ...
    headerBg: "bg-amber-500", badge: "bg-amber-100 text-amber-700",
    icon: <AlertTriangle className="h-5 w-5 text-white" />,
    label: "HIGH STRESS", labelColor: "text-amber-600",
    btnPrimary: "bg-emerald-500 hover:bg-emerald-600",
  ...
  return {
    ...
    headerBg: "bg-yellow-500", badge: "bg-yellow-100 text-yellow-700",
    icon: <AlertTriangle className="h-5 w-5 text-white" />,
    label: "CAUTION", labelColor: "text-yellow-600",
    btnPrimary: "bg-emerald-500 hover:bg-emerald-600",
...
const biasLabels: Record<string, string> = {
  FOMO_BUYING: "FOMO Spending — your last 3 big transactions happened on weekends after peers spent first",
  PANIC_SELLING: "Panic Spending — stress may be driving this decision, not need",
  PRESENT_BIAS: "Present Bias — prioritising right now over upcoming fees",
  LOSS_AVERSION: "Loss Aversion — fear of missing out is influencing this",
  HERD_BEHAVIOR: "Herd Behaviour — others spending doesn't mean you should",
  MENTAL_ACCOUNTING: "Mental Accounting — don't treat this money differently just because of its source",
};
  ...
  return (
    <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 ${
      ...
      <div>
        ...
        <p className={`text-xs ${isNecessity ? "text-emerald-700" : "text-orange-700"}`}>
          {isNecessity
...
function StressBar({ score, barColor }: { score: number; barColor: string }) {
  ...
  const label =
    score >= 80 ? "EXTREME PANIC" :
    score >= 60 ? "HIGH STRESS" :
    score >= 30 ? "MODERATE" : "CALM";
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-[10px] text-gray-500">
        <span className="font-semibold uppercase tracking-wide">Layer 1 — Market Pulse</span>
        <span className="font-bold">{score}/100 · {label}</span>
      </div>
...
function ExamBadge({ daysToExam, lambdaT }: { daysToExam?: number; lambdaT?: number }) {
  ...
  return (
    <div className={`flex items-start gap-2 rounded-xl border px-3 py-2 ${
      ...
      <BookOpen className={`h-4 w-4 shrink-0 mt-0.5 ${urgent ? "text-red-500" : "text-gray-400"}`} />
      <div>
        <p className={`text-[10px] font-bold uppercase tracking-wide ${urgent ? "text-red-600" : "text-gray-500"}`}>
          Layer 3 — Academic Stress
        </p>
        <p className={`text-xs ${urgent ? "text-red-700 font-semibold" : "text-gray-600"}`}>
          Exams in {daysToExam} days
          {lambdaT !== undefined && (
            <span className="font-normal text-gray-400"> · λt = {lambdaT.toFixed(2)} — Zelta is {lambdaT < 0.9 ? "more conservative" : "standard"} right now</span>
          )}
        </p>
      </div>
    </div>
...
function RunwayCard({ freeCash, amount, weeksRunway }: {
  freeCash: number; amount: number; weeksRunway?: number;
}) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4 space-y-3">
      <p className="text-[10px] font-bold uppercase tracking-wide text-gray-500">Runway Impact</p>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className={`rounded-xl border p-3 text-center ${
          ...
          <p className={`text-[10px] font-medium uppercase ${danger ? "text-red-600" : "text-orange-600"}`}>After spend</p>
          <p className={`text-xl font-bold ${danger ? "text-red-700" : "text-orange-700"}`}>{daysAfter}d</p>
          <p className={`text-[9px] ${danger ? "text-red-500" : "text-orange-500"}`}>{fmt(Math.max(0, freeCash - amount))}</p>
        </div>
      </div>
      <div>
        <div className="flex justify-between text-[10px] text-gray-400 mb-1">
          <span>Runway remaining after spend</span>
          <span className={`font-bold ${danger ? "text-red-600" : "text-orange-500"}`}>{pct}%</span>
        </div>
        <div className="h-2 w-full rounded-full bg-gray-200 overflow-hidden">
          <div
            className={`h-2 rounded-full transition-all duration-700 ${danger ? "bg-red-500" : "bg-orange-400"}`}
            style={{ width: `${pct}%` }}
          />
        </div>
        {danger && (
...
function GigConnection({ gigs, shortfall }: { gigs: GigSuggestion[]; shortfall: number }) {
    ...
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
      ...
      <div className="space-y-2">
        {gigs.slice(0, 3).map((g, i) => (
          <div key={i} className="rounded-xl border border-emerald-100 bg-white p-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <div className="flex items-center gap-1 text-[10px] text-gray-500">
                <Clock className="h-3 w-3" />{g.hours}h
              </div>
              <div className="flex items-center gap-1 text-sm font-bold text-emerald-600">
                <DollarSign className="h-3.5 w-3.5" />{fmt(g.payout).replace("₦","₦")}
              </div>
...
export default function InterceptModal({
  ...
  const biasLabel = biasDetected ? biasLabels[biasDetected.toUpperCase()] ?? `Bias detected: ${biasDetected}` : null;
  ...
  const handleContinue = async () => {
    ...
    try { await onContinue(); } finally { setContinueLoading(false); }
  };

  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center px-3"
      onClick={(e) => { if (e.target === e.currentTarget) { onClose?.(); onCancel(); } }}
    >
      <div className={`w-full max-w-md rounded-t-3xl sm:rounded-3xl ${cfg.bg} ${cfg.border} border-2 shadow-2xl overflow-hidden`}>
        ...
        <div className={`${cfg.headerBg} px-5 py-4 flex items-center justify-between`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/80 text-xs">{cfg.label} — Decision checkpoint</p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 space-y-3 max-h-[72vh] overflow-y-auto">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400 mb-2">You attempted</p>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold text-gray-900">{fmt(amount)}</p>
                <p className="text-xs text-gray-500 capitalize">{category}{description ? ` — ${description}` : ""}</p>
              </div>
          ...
          {bqAlert && (
            <div className="rounded-2xl border border-gray-200 bg-white p-4 flex gap-3">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 mb-1">ZELTA says</p>
          ...
          {stressScore > 0 && (
            <div className="rounded-2xl border border-gray-200 bg-white p-4">
              ...
              {marketTitle && (
                <p className="mt-1 text-[10px] text-gray-400">Market: {marketTitle}</p>
              )}
            </div>
          )}
          ...
          {safeAmount !== undefined && safeAmount > 0 && safeAmount < amount && (
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3 flex gap-2">
              ...
              <p className="text-xs text-emerald-800">
                Kelly model suggests capping at <span className="font-bold">{fmt(safeAmount)}</span> instead of {fmt(amount)}.
              </p>
            </div>
          )}
